...
def roster_import(
    ...
    org: str = typer.Option(
        ..., "--org", help="GitHub organization (e.g., 'soc-cs3550-f25')"
    ),
    skip_duplicates: bool = typer.Option(
        True,
        "--skip-duplicates/--fail-on-duplicates",
...
def roster_list(
    ...
    org: Optional[str] = typer.Option(
        None, "--org", help="Filter by GitHub organization"
    ),
    status: str = typer.Option(
        "active", "--status", help="Filter by status (active, inactive, dropped)"
    ),
...
def roster_add(
    ...
    github: Optional[str] = typer.Option(
        None, "--github", help="GitHub username (optional)"
    ),
):
    """
...
def roster_sync(
    ...
    if result.unlinked_repos:
        ...
        if len(result.unlinked_repos) > 10:
            console.print(
                f"   ... and {len(result.unlinked_repos) - 10} more", style="yellow dim"
            )
