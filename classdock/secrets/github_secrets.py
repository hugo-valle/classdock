...
class GitHubSecretsManager:
    ...
    def _get_github_token(self) -> str:
        ...
        try:
            ...
            if token:
                ...
                logger.warning("Token manager returned None, trying fallback methods")
                return self._get_github_token_fallback()

        except Exception as e:
            logger.warning(f"Error using token manager: {e}, trying fallback methods")
            return self._get_github_token_fallback()
    ...
    def encrypt_secret(self, public_key: str, secret_value: str) -> str:
        ...
        """
        logger.warning("encrypt_secret method is deprecated - using GitHub CLI instead")
        return secret_value
    ...
    def secret_needs_update(
        self, secret_info: Optional[Dict[str, str]], max_age_days: int = 90
    ) -> bool:
    ...
    def add_secret_to_repo(
        ...
        logger.info(f"Adding secret '{secret_name}' to repository: {repo_full_name}")
        ...
        if existing_secret and not force_update:
            if not self.secret_needs_update(existing_secret, max_age_days):
                logger.info(f"Secret '{secret_name}' is up to date in {repo_full_name}")
                return True
        ...
        try:
            ...
            logger.info(f"✅ {action} secret '{secret_name}' in {repo_full_name}")
            return True

        except subprocess.CalledProcessError as e:
            logger.error(f"GitHub CLI error for {repo_full_name}: {e.stderr.strip()}")
            return False
        except Exception as e:
    ...
    def validate_token_format(
        self, token_value: str, skip_validation: bool = False
    ) -> bool:
    ...
    def add_secrets_from_global_config(
        ...
        try:
            ...
            if not self.global_config.step_manage_secrets:
                logger.info("Secret management is disabled (STEP_MANAGE_SECRETS=false)")
                return True
            ...
            if repo_urls is None:
                ...
                if not repo_urls:
                    logger.warning("No repositories discovered for secrets management")
                    return True
            ...
            for secret_config in self.global_config.secrets_config:
                ...
                if secret_config.uses_centralized_token():
                    ...
                    try:
                        ...
                        logger.debug(f"Loaded secret value from file: {token_file}")
                    except Exception as e:
...
def add_secrets_to_students(
    ...
    try:
        ...
        if batch_file:
            ...
            logger.info("Processing secrets management from assignment configuration")
            ...
            secret_lines = [
                line.strip() for line in secrets_config.split("\n") if line.strip()
            ]
            if not secret_lines:
                logger.warning("No secret configurations found in SECRETS_CONFIG")
                return True
            ...
            if len(secret_config_parts) >= 3:
                ...
                max_age_days = (
                    int(secret_config_parts[3]) if len(secret_config_parts) > 3 else 90
                )
                skip_validation = not (
                    secret_config_parts[4].lower() == "true"
                    if len(secret_config_parts) > 4
                    else True
                )
            else:
                logger.error(f"Invalid SECRETS_CONFIG format: {secret_lines[0]}")
                return False

            # Use INSTRUCTOR_TOKEN_FILE if specified
            instructor_token_file = config.get("INSTRUCTOR_TOKEN_FILE", token_file)
            ...
            if not token_path.exists():
                ...
                logger.info("Create the token file to enable secrets management")
                if dry_run:
            ...
            # Validate token format
            try:
                token_value = manager.get_instructor_token(instructor_token_file)
            ...
            if dry_run:
                logger.info("✅ Dry run: Secrets management configuration is ready")
                logger.info(
