...
class GitHubClassroomAPI:
    ...
    def validate_token_scopes(self) -> Dict[str, bool]:
        ...
        try:
            ...
            return {
                ...
                or "admin:org" in scope_list
                or "repo" in scope_list,
                    ...
                    if token_data and isinstance(token_data, dict):
                        ...
                        if stored_expires_at:
                            try:
                                ...
                                stored_days_remaining = (expiry_date - now).days
                            except Exception as e:
                                logger.debug(
                                    f"Error parsing stored expiration date: {e}"
                                )
                except Exception as e:
                    logger.debug(f"Could not retrieve stored token expiration: {e}")

                return {
                    "is_expired": True,
                    "is_valid": False,
                    "expires_at": stored_expires_at,
                    "days_remaining": stored_days_remaining
                    if stored_days_remaining is not None
                    else 0,
                    "token_type": "expired",
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            if status_code == 401:
                ...
                error_message += "\n  3. Run: classdock config set-token <your-token>"
            ...
            raise GitHubClassroomAPIError(
                error_message, response=error_response, status_code=status_code
            )
    ...
    def parse_classroom_url(self, classroom_url: str) -> Tuple[str, str]:
        ...
        if not match:
            raise ValueError(f"Invalid classroom URL path format: {parsed.path}")
    ...
    def get_classroom_assignments(self, classroom_id: int) -> List[Dict]:
        ...
        response = self._make_request("GET", f"/classrooms/{classroom_id}/assignments")
    ...
    def get_assignment_metadata(
        ...
        try:
            ...
            assignments = self.get_classroom_assignments(target_classroom["id"])
            ...
            for assignment in assignments:
                ...
                if (
                    assignment_name in assignment_slug
                    or assignment_slug in assignment_name
                    or assignment_name in assignment_title.lower()
                ):
            ...
            if target_assignment:
                ...
                metadata["assignment_type"] = (
                    "group"
                    if target_assignment.get("max_teams", 1) > 1
                    else "individual"
                )
                ...
                try:
                    ...
                    logger.debug(f"Could not get template repository info: {e}")
    ...
    def _infer_template_repository(
        self, organization: str, assignment_name: str, assignment_data: Dict
    ) -> str:
    ...
    def find_assignment_by_url(
        ...
        try:
            ...
            logger.info(f"Looking for assignment with ID pattern: {assignment_id}")
            ...
            for classroom in classrooms:
                if (
                    classroom.get("organization", {}).get("login")
                    == github_organization
                ):
                    logger.debug(f"Checking classroom: {classroom.get('name')}")

                    # Get assignments for this classroom
                    assignments = self.get_classroom_assignments(classroom["id"])
                    ...
                    for assignment in assignments:
                        ...
                        assignment_slug = assignment.get("slug", "")
                        if (
                            assignment_id in assignment_slug
                            or assignment_slug in assignment_id
                        ):
                            logger.info(
    ...
    def discover_student_repositories(
        self,
        classroom_url: str,
        ...
        try:
            ...
            if expiration_info.get("is_expired"):
                ...
                logger.error("     ✓ repo (Full control of private repositories)")
                logger.error("     ✓ read:org (Read organization data)")
                logger.error("  3. Update your token:")
                logger.error("     classdock config set-token <your-new-token>")
                logger.error("")
                raise GitHubClassroomAPIError(
                    "GitHub token has expired. Please generate a new token and update it using 'classdock config set-token'",
                    status_code=401,
                )
            ...
            if not expiration_info.get("is_valid"):
                ...
                logger.error("")
                raise GitHubClassroomAPIError(
                    f"GitHub token is invalid: {error_msg}", status_code=401
                )
            ...
            if expiration_info.get("days_remaining") is not None:
                ...
                logger.debug("✓ Token is valid (classic token with no expiration)")
            ...
            if not scope_info.get("has_read_org"):
                ...
                logger.warning(
                    "Current scopes: " + ", ".join(scope_info.get("scopes", []))
                )
            ...
            if not assignment_prefix:
                logger.warning("Could not extract assignment prefix from classroom URL")
                logger.info(
                    "Will attempt to discover repositories using organization listing"
                )

            logger.info(f"Discovering repositories with prefix: {assignment_prefix}")
            logger.info(f"From organization: {github_organization}")

            # Get all repositories from the organization
            repositories = self._get_organization_repositories(github_organization)
            ...
            logger.info(f"Discovered {len(student_repos)} student repositories")
    ...
    def _get_organization_repositories(
        self, organization: str, per_page: int = 100
    ) -> List[Dict]:
        """
        ...
        while True:
            ...
            if page > 50:  # Max 5000 repositories
                logger.warning("Reached maximum page limit for repository discovery")
                break

        logger.debug(f"Found {len(repositories)} total repositories in organization")
        return repositories

    def _filter_student_repositories(
        self,
        repositories: List[Dict],
        assignment_prefix: Optional[str],
        ...
        for repo in repositories:
            ...
            if not assignment_prefix:
                ...
                if "-" in repo_name and not any(
                    keyword in repo_name.lower()
                    for keyword in ["template", "instructor", "classroom"]
                ):
