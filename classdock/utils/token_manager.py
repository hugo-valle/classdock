...
class GitHubTokenManager:
    ...
    def __init__(self):
        self.config_dir = Path.home() / ".config" / "classdock"
    ...
    def get_github_token(self):
        ...
        1. User config file (~/.config/classdock/token_config.json)
        2. System keychain (OS-specific secure storage)
    ...
    def save_token(self, token, expires_at=None, scopes=None):
        ...
        try:
            ...
            if expires_at:
                ...
                logger.debug(f"Using manually provided expiration: {expires_at}")
            elif token_data.get("expires_at"):
                # Mark that this came from GitHub API
                token_data["expires_at_source"] = "github_api"
                logger.debug(f"Using GitHub API expiration: {token_data['expires_at']}")
            else:
    ...
    def setup_new_token(self):
        ...
        """
        from ..utils.ui_components import Colors, print_colored

        print_colored("🔐 GitHub Token Setup Required", Colors.CYAN)
        print_colored(
            "\nTo use classdock, you need a GitHub Personal Access Token.", ""
        )
        print_colored("\n📋 Required token permissions:", Colors.YELLOW)
        print_colored("  • repo (full control of repositories)", "")
        print_colored("  • read:org (read organization membership)", "")
        print_colored("  • admin:repo_hook (repository hooks)", "")
        print_colored("  • workflow (update GitHub Actions workflows)", "")
        ...
        while True:
            ...
            if not (token.startswith("ghp_") or token.startswith("github_pat_")):
                print_colored(
                    "⚠️ Token should start with 'ghp_' (classic) or 'github_pat_' (fine-grained)",
                    Colors.YELLOW,
                )
            ...
            if token_data:
                ...
                if token_data.get("token_type") == "classic" and not token_data.get(
                    "expires_at"
                ):
                    print_colored(
                        "ℹ️ Classic token detected - API doesn't provide expiration info",
                        Colors.CYAN,
                    )
                    token_data["expires_at"] = self._ask_token_expiration()
                elif token_data.get("token_type") == "fine-grained":
                    print_colored(
                        "ℹ️ Fine-grained token detected - expiration tracked automatically",
                        Colors.GREEN,
                    )
    ...
    def _verify_and_get_token_info(self, token):
        ...
        try:
            ...
            if user_response.status_code != 200:
                logger.error(f"Token verification failed: {user_response.status_code}")
                if user_response.status_code == 401:
                    logger.error(
                        "❌ Authentication failed - token may be invalid or expired"
                    )
                elif user_response.status_code == 403:
                    logger.error("❌ Forbidden - token may lack required permissions")
            ...
            if token.startswith("github_pat_"):
                ...
                if expires_header:
                    try:
                        ...
                        token_expires = (
                            expires_header  # Store raw value if parsing fails
                        )
            else:
            ...
            token_data = {
                ...
                "rate_limit_remaining": user_response.headers.get(
                ...
                "api_headers": {
                    ...
                    "x-github-media-type": user_response.headers.get(
                        "x-github-media-type"
                    ),
                    "x-github-enterprise-version": user_response.headers.get(
                        "x-github-enterprise-version"
                    ),
                    "github-authentication-token-expiration": user_response.headers.get(
                        "github-authentication-token-expiration"
                    ),
                    "x-oauth-token-expires": user_response.headers.get(
    ...
    def _get_fine_grained_permissions(self, headers):
        ...
        try:
            response = requests.get(
                "https://api.github.com/user/repos?type=owner&per_page=1",
                headers=headers,
                timeout=10,
            )
            if response.status_code == 200:
                ...
                if repos and len(repos) > 0:
                    ...
                    if webhook_response.status_code == 200:
                        ...
                        permission_details["admin:repo_hook"] = {
                            "status": "granted",
                            "tested_repo": repo_full_name,
                        }
                    else:
                        permission_details["admin:repo_hook"] = {
                            "status": "denied",
                            "code": webhook_response.status_code,
                        }
        except Exception as e:
            ...
            if response.status_code == 200:
                ...
                if repos and len(repos) > 0:
                    ...
                    if actions_response.status_code == 200:
                        ...
                        permission_details["workflow"] = {
                            "status": "denied",
                            "code": actions_response.status_code,
                        }
        except Exception as e:
    ...
    def _check_expiration_warning(self, token_data):
        ...
        try:
            ...
            if days_until_expiry <= 7:
                logger.warning(f"⚠️ Token expires in {days_until_expiry} days!")
    ...
    def _ask_storage_preference(self):
        ...
        print_colored("3. Config file (~/.config/classdock/) (recommended)", "")
    ...
    def _ask_token_expiration(self):
        ...
        print_colored(
            "Please enter the expiration date you set when creating the token.",
            Colors.CYAN,
        )
        ...
        while True:
            expiry_input = input(
                "\n📅 Enter expiration date (YYYY-MM-DD) or press Enter to skip: "
            ).strip()

            if not expiry_input:
                print_colored(
                    "⚠️ Skipping expiration date - you won't get renewal reminders",
                    Colors.YELLOW,
                )
                return None
            ...
            try:
                ...
                expiry_iso = expiry_date.replace(tzinfo=timezone.utc).isoformat()
                print_colored(f"✅ Expiration date set: {expiry_input}", Colors.GREEN)
                return expiry_iso
            except ValueError:
                print_colored(
                    "❌ Invalid date format. Please use YYYY-MM-DD (e.g., 2024-12-31)",
                    Colors.RED,
                )
    ...
    def _store_token(self, token, token_data, storage_choice):
        ...
        if storage_choice == 1:
            ...
            if platform.system() != "Darwin":
                print_colored("❌ Keychain storage only available on macOS", Colors.RED)
                # Fallback to config
            ...
            print_colored(f"✅ Token stored in: {self.config_file}", Colors.GREEN)
            print_colored(
                "✅ Config file provides the best control with expiration tracking",
                Colors.GREEN,
            )
