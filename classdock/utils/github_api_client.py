...
class GitHubAPIClient:
    ...
    def get_classroom_by_id(self, classroom_id: int) -> Optional[ClassroomInfo]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/classrooms/{classroom_id}",
    ...
    def find_classroom_by_url_segment(
        self, url_segment: str
    ) -> Optional[ClassroomInfo]:
        """
    ...
    def extract_classroom_data_from_url(self, classroom_url: str) -> Dict[str, str]:
        ...
        try:
            ...
            if not matching_classroom:
                result[
                    "error"
                ] = f"No classroom found matching URL segment: {classroom_id_segment}"
                return result
            ...
            assignments = self.list_classroom_assignments(matching_classroom.id)
    ...
    def _extract_org_from_classroom_url(self, classroom_url: str) -> str:
        ...
        org_match = re.search(r"github\.com/organizations/([^/]+)/", classroom_url)
        if org_match:
